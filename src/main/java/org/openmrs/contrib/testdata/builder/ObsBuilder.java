...
public class ObsBuilder extends TestDataBuilder<Obs> {
    ...
    public Obs save() {
        ...
        if (created.getEncounter() != null && !created.getEncounter().getObs().contains(created)) {
            created.getEncounter().addObs(created);
            testDataManager.getEncounterService().saveEncounter(created.getEncounter());
        }
        return created;
    }
